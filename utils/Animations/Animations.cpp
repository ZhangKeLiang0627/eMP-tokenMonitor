...
void lv_my_anim_cb_pop_up(void *obj, int val)
{
    ...
    lv_coord_t y_start = (lv_coord_t)((intptr_t)lv_obj_get_user_data(target_obj));
...
void lv_my_anim_fall_down(lv_obj_t *obj, uint16_t time, uint16_t delay, bool del)
{
    ...
    else
    {
        lv_anim_set_ready_cb(&a, [](lv_anim_t *a)
                ...
                lv_coord_t y_start = (lv_coord_t)((intptr_t)lv_obj_get_user_data(obj));
...
void lv_anim_drop_out(lv_obj_t *obj, uint16_t time, uint16_t delay, bool del, lv_coord_t x, lv_coord_t y)
{
    lv_anim_t a;
    int16_t p;
    // lv_coord_t x = -30;
    ...
    p = lv_obj_get_style_opa(obj, 0);
    if (p != LV_OPA_TRANSP)
    {
        lv_anim_set_exec_cb(&a, [](void *data, int32_t val)
                            { lv_obj_set_style_opa((lv_obj_t *)data, val, 0); });
        lv_anim_set_values(&a, p, LV_OPA_TRANSP);
        lv_anim_start(&a);
    }
...
// 旋转动画回调函数
static void lv_anim_rotate_cb(void *obj, int32_t value)
{
    ...
    lv_obj_set_style_transform_pivot_x(btn, w / 2, LV_PART_MAIN);
    lv_obj_set_style_transform_pivot_y(btn, h / 2, LV_PART_MAIN);

    // 设置旋转角度，LVGL中角度以0.1度为单位（3600 = 360.0度）
    lv_obj_set_style_transform_angle(btn, value, LV_PART_MAIN);
}
void lv_anim_center_rotate(lv_obj_t *obj, int32_t angle, uint16_t time, uint16_t delay)
{
    lv_anim_t a;
    lv_anim_del(obj, NULL);
    lv_anim_init(&a);
    ...
    lv_anim_set_exec_cb(&a, (lv_anim_exec_xcb_t)lv_anim_rotate_cb);
    lv_anim_set_path_cb(&a, my_anim_path_ease_in_out);
    lv_anim_set_time(&a, time);
    lv_anim_set_delay(&a, delay);
    lv_anim_start(&a);
